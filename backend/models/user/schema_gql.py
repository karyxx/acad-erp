...
class Query:
    @strawberry.field
    def get_user_by_username(self, info: strawberry.Info, username: str) -> Optional[Users]:
        session: Session = info.context["session"]
        user = get_user_by_username(session, username)
        if user:
            return Users(id=user.id, username=user.username, email=user.email)
        return None

    @strawberry.field
    def get_user_by_id(self, info: strawberry.Info, user_id: int) -> Optional[Users]:
        session: Session = info.context["session"]
...
class Mutation:
    @strawberry.mutation
    def create_user(self, info: strawberry.Info, username: str, password: str, email: Optional[str] = None) -> Users:
        session: Session = info.context["session"]
        ...
        return Users(id=created_user.id, username=created_user.username, email=created_user.email)
